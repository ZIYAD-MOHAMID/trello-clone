...
export default function Moddal() {
    ...
    const [image, setImage, addTask] = useBoardStore((state) => [
        state.image,
        state.setImage,
        state.addTask,
    ])
    ...
    const [newTaskType] = useBoardStore((state) => [
        state.newTaskType
    ])
    ...
    return (
        ...
        <Transition appear show={isOpen} as={Fragment}>
            <Dialog as='form' onSubmit={handlarSubmit} className='flex justify-center z-10 absolute top-2/4 left-2/4 w-full h-full -translate-x-2/4 -translate-y-2/4' onClose={closeModal}>
                <Transition.Child
                    as={Fragment}
                    ...
                    <div className="flex min-h-full items-center  p-4 text-center justify-center">
                        ...
                        >
                            <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all ">
                                ...
                                <div>
                                    <button disabled={!newTaskInput} type='submit' className='inline-flex justify-center round-md border border-transparent bg-blue-100 px-4 py-2 text-sm font-medium text-blue-900 hover:bg-blue-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 disabled:bg-gray-100 disabled:text-gray-300 disabled:cursor-not-allowed translate-x-full mt-5'>
                                        Add Task
                                    </button>
                                </div>
                            </Dialog.Panel>
